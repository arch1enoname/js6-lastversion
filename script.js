...
function showForm() {
    ...
    let ag = document.Form.ageLimit.selectedIndex;
    let ag1 = document.Form.ageLimit.options[ag].value;

    let rat = document.forms.Form.rating.value;

    let ye = document.forms.Form.year.value;

    let len = document.forms.Form.length.value;
    ...
    for (let i = 0; i < films.length; i++) {
        ...
        if (c4 != -1) {
            ...
            if (((films[i].genres.indexOf(g1) != -1) || (films[i].genres.indexOf(g2) != -1) || (films[i].genres.indexOf(g3) != -1))) {
                if(ag1 == films[i].age && rat < films[i].rating && len > films[i].length && ye < films[i].year) {
                    getCard(i);
                    if(c1 == -1);
                    c1 = i;
                } else if (c2 == -1){
